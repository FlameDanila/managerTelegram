...
async Task GetContactAsync(ITelegramBotClient botClient, MessageType messageType, Message message)
{
    ...
    {
        new KeyboardButton [] {"Да, это я", "Нет, это не я"},
    }
    )
    { ResizeKeyboard = true };
    ...
    for (int i = 0; i < user.Rows.Count; i++)
    {
        ...
        {
            var names = await Select($"select name from usersDelfaTelegram where telegramId = '{chatId}'");
            await botClient.SendTextMessageAsync(chatId, "Это вы?\n" + $"{names.Rows[0][0]}", replyMarkup:authorize) ;
        }
    }
